...
The [frontend](https://verification.kidswebservices.com/en/verification) is also very complex and makes use of obfuscated JavaScript which made proper analysis of it quite difficult. They do appear to do some sort of encryption^[I found mentions of this in the frontend sources. The best way to dig into this is to configure your developer tools to break on XHR so you can view the stack up to the point where the image data is POSTed to the backend.] of the image data prior to `POST`ing it, but the obfuscation of their app made it difficult to work out what exactly was happening.
